...
params={};
    ...
    for i=1:length(logfiles)
        ...
        %thirds (excluding last trial so it's divisible by 3)
        stepsizethirds = (ntrials-1)/3;
        onsets(i).stimOnset_third1 = onsets(i).stimOnset(1:stepsizethirds); % 1st third
        onsets(i).stimOnset_third2 = onsets(i).stimOnset((1+stepsizethirds):(2*stepsizethirds)); % 2nd third 
        onsets(i).stimOnset_third3 = onsets(i).stimOnset((1+2*stepsizethirds):(ntrials-1));  % 3nd third
        
        
        onsets(i).feedbackOnset_third1 = onsets(i).feedbackOnset(1:stepsizethirds); % 1st third
        onsets(i).feedbackOnset_third2 = onsets(i).feedbackOnset((1+stepsizethirds):(2*stepsizethirds)); % 2nd third 
        onsets(i).feedbackOnset_third3 = onsets(i).feedbackOnset((1+2*stepsizethirds):(ntrials-1));  % 3nd third
