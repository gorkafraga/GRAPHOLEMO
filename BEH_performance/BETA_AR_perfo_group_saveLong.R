##################################################
# SAVE PERFORMANCE PER BIN  SUMMARY IN LONG FORMAT 
# - Read logs
# - Summarize accu and RTs 
# - Save in a long formatted table
##############################################
#Load libraries
...
#set ins and outs
dirinput <-"O:/studies/allread/mri/analysis_GFG/stats/task/performance/logs_no0e_s80" # no  / at the end 
diroutput <-"O:/studies/allread/mri/analysis_GFG/stats/task/performance/logs_no0e_s80"
...
#loop thru files
setwd(dirinput)
files <-dir(dirinput,paste0('AR.*.*.txt'),recursive=TRUE,ignore.case = TRUE,full.names = FALSE)
...
for (i in 1:length(files)){
  ...
  #subject <- substr(sapply(strsplit(files[i],"/"), `[`,2),1,6)#subject ID are the 6 first characters from filename,i.e., last element after splitting fullpath name...
  subject <-  sapply(strsplit(files[i],"/"), `[`,1)
...
dlongFull <-dlong %>% complete(subjID,nesting(block,fb,quartile),fill = list(n = 0)) # Make explicit the missing values ! nw length should be nsubjects x blocks x quartile x fb 
 write.csv(dlongFull,paste(diroutput,"/Performance_long.csv",sep=""),row.names = FALSE)
