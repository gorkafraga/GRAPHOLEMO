...
#set ins and outs
task <- "fbl_b"
dirinput <- paste0("O:/studies/grapholemo/analysis/LEMO_GFG/beh/modeling/analysis_n39/",task,"/data_preprocessed/Copy_source_files") 
diroutput <-paste0("O:/studies/grapholemo/analysis/LEMO_GFG/beh/modeling/analysis_n39/",task,"/data_plots/") 
...
#loop thru files
setwd(dirinput)
files <- dir(path = dirinput,pattern = paste0('*.',task,'.*.txt'),recursive = TRUE,ignore.case = TRUE)
...
ggsave(outputname,combo,width = 350, height = 310, dpi=300, units = "mm",bg = "white")
cat(i,"-done.\n")
setwd(dirinput)
  }}         
