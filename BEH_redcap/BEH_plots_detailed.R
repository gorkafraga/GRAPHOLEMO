...
diroutput <- 'O:/studies/grapholemo/analysis/LEMO_GFG/beh/plots_cognitiveTests/per_gender'
setwd(diroutput)
...
for (i in 1:length(tests)){
  ...
  for (ii in 1:length(typescore)){
    ...
    gfig <- gfig + facet_grid(~Sex)
    
    if (length(vars2read) > 2) {
      ggsave(plot = gfig,filename = paste0(diroutput,"/Boxplots_perGender_",testname,"_",typescore[ii],".jpg"),height=250,width = 70*length(vars2read),dpi = 150,units="mm")
    } else if (length(vars2read)==1) {
      ggsave(plot = gfig,filename = paste0(diroutput,"/Boxplots_perGender_",testname,"_",typescore[ii],".jpg"),height=250,width = 160*length(vars2read),dpi = 150,units="mm")
    } else {
      ggsave(plot = gfig,filename = paste0(diroutput,"/Boxplots_perGender_",testname,"_",typescore[ii],".jpg"),height=250,width = 100*length(vars2read),dpi = 150,units="mm")
    }
