...
#inputs
fileinput <- "FPVS_gathered_long_newGroups.sav" 
dirinput <- "N:/Users/gfraga/_Misc/CHRISTINA/FPVS_redo/stats_newGroups" 
diroutput <-  "N:/Users/gfraga/_Misc/CHRISTINA/FPVS_redo/stats_newGroups/groupELFESLRTcomb"
outlierCoeff <- 3 # or 1.5 


measurelist <- c('snr','bcAmps','zscores', 'specNorm')
...
for (mm in 1:length(measurelist)){
    ...
    dat <- dat[which(dat$groupELFESLRTcomb=='Typical' | dat$groupELFESLRTcomb== 'Poor'|dat$groupELFESLRTcomb== 'Gap'),]
    ...
    for (i in 1:length(condition)){
        ...
        outlier_cases <- paste0(paste(dat$subject[outlier_idx],dat$groupELFESLRTcomb[outlier_idx],dat$cond[outlier_idx],dat$hemisphere[outlier_idx],sep="_"),collapse=",")
...
# save as xls
openxlsx::write.xlsx(df,file=paste0(diroutput,'/Outliers_boxplotStats_newGroups_',measure,'.xlsx'))
haven::write_sav(dat, paste0(diroutput,'/FPVS_gathered_long_newGroups_',measure,'_outliers.sav'))
