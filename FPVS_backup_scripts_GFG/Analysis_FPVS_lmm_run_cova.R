...
if (exclude_dataOutliers == 1){
  diroutput       <- paste0('N:/Users/gfraga/_Misc/CHRISTINA/FPVS_redo/stats/lmm_GFG/exclDataOutliers/Base_',mybase)
}else if (exclude_dataOutliers == 0) {
  diroutput       <- paste0('N:/Users/gfraga/_Misc/CHRISTINA/FPVS_redo/stats/lmm_GFG/exclOnly_modelOutliers/Base_',mybase)
}
dir.create(diroutput)

# File to read
fileinput       <- 'FPVS_gathered_long_outliers.sav'
...
formula_fixed   <- as.formula('value ~ hemisphere*group*grade*cond + rias_nix_T1')  
...
dat <- dplyr::filter(dat , (base == mybase) & (type== mytype) & (group== 'Typ' | group == 'Poor') )
...
outcol[as.numeric(unlist(strsplit(excludedRows_id,','))),1] <- 1
...
if(exists('additionalContrasts')){ 
  ...
  newcon <- emmeans(fit,as.formula('pairwise ~ hemisphere | group'),adjust="Tukey") 
  additionalContrasts[[length(additionalContrasts)+1]]  <- newcon
  newcon <- emmeans(fit,as.formula('pairwise ~ group | hemisphere'),adjust="Tukey") 
  additionalContrasts[[length(additionalContrasts)+1]]  <- newcon
  newcon <- emmeans(fit,as.formula('pairwise ~  group | hemisphere | cond'),adjust="Tukey") 
  additionalContrasts[[length(additionalContrasts)+1]]  <- newcon  
