...
dirinput <- "N:/Users/gfraga/_Misc/CHRISTINA/FPVS_redo/data"
...
listgroups <- c('Poor_2nd','Typ_2nd','Poor_3rd','Typ_3rd','Gap_2nd','Gap_3rd')
...
for (gg in 1:length(listgroups)){
  ...
  files <-  dir(pattern=paste0('*.',listgroups[gg],'.*_Pool_ElRankTop3_FFT_.*.csv')) # find all files
        ...
        # Gather unique data types (subject, group, type, electrode, condition)
        # groups <- list()
        # types <- list()
        # conditions <- list()
        # chans <- list()
        # for (f in 1:length(files)) {
        #     # retrieve condition from file name 
        #     fnamesplit <-strsplit(files[f],"_","")
        #     conditions[[f]] <- gsub(".csv","",fnamesplit[[1]][as.numeric(lapply(fnamesplit,grep, pattern="*.csv"))]) # splits file name by "_", applies a function to find the piece containing extension and deletes the extension from that part 
        #     groups[[f]] <- fnamesplit[[1]][4] #4th element of the list (of the split filename) is the group description!
        #     types[[f]]  <- fnamesplit[[1]][6] #6th element of the list (of the split filename) is the type description!
        #     conditions[[f]] <- gsub(".csv","",fnamesplit[[1]][as.numeric(lapply(fnamesplit,grep, pattern="*.csv"))]) # splits file name by "_", applies a function to find the piece containing extension and deletes the extension from that part 
        #     chans[[f]] <- gsub("ch","",fnamesplit[[1]][as.numeric(lapply(fnamesplit,grep, pattern="*ch"))])
        # }
        # group <- unique(unlist(groups))
        # type <- unique(unlist(types))
        # condition <- unique(unlist(conditions))
        # chan <- unique(unlist(chans))
        # 
        ...
        #AVgathered <- list()
        #LinPLgathered <- list()
        #stringsgathered <- list()
        
        for (f in 1:length(files)){
          
          #read as data frame
          #df <- read.delim(files[f],check.names=FALSE,sep=",")
          ...
          type <- fnamesplit[[1]][6] #6th element of the list (of the split filename) is the type description!
...
table2save$Class <- sapply(strsplit(table2save$group,'_'),"[[", 2)
table2save$Group <- sapply(strsplit(table2save$group,'_'),"[[", 1)
...
filename_Final <- paste0("FPVS_gatheredBins",".csv",sep="")
