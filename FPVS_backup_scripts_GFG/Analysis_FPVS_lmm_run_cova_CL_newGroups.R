...
Packages <- c("nlme","emmeans","haven","ggplotify","dplyr","readxl","sjPlot","cowplot") #Load libraries (you mast install packages before)
...
exclude_dataOutliers <- 1
mybase="W"
mytype="bcAmps"

# Edit Paths
dirinput        <- 'N:/Users/gfraga/_Misc/CHRISTINA/FPVS_redo/stats_newGroups/group1625_SLRTsep_OR'
#output dir
if (exclude_dataOutliers == 1){
  diroutput       <- paste0('N:/Users/gfraga/_Misc/CHRISTINA/FPVS_redo/stats_newGroups/group1625_SLRTsep_OR/lmm_GFG/exclDataOutliers/Base_',mybase)
}else if (exclude_dataOutliers == 0) {
  diroutput       <- paste0('N:/Users/gfraga/_Misc/CHRISTINA/FPVS_redo/stats_newGroups/group1625_SLRTsep_OR/lmm_GFG/exclOnly_modelOutliers/Base_',mybase)
}
dir.create(diroutput)

# File to read
fileinput       <- 'FPVS_gathered_long_newGroups_bcAmps_outliers.sav'
...
formula_fixed   <- as.formula('value ~ hemisphere*group1625_SLRTsep_OR*grade*cond + rias_nix_T1')  
...
dat <- dplyr::filter(dat , (base == mybase) & (type== mytype) & (group1625_SLRTsep_OR== 'Typical' | group1625_SLRTsep_OR == 'Poor') )

if (exclude_dataOutliers==1){
  # if required save data-based outliers excluded 
  ex <- as.data.frame(dat$rowID[which(dat$Outliers_bcAmps_withinCond ==1)])
  colnames(ex)[1] <- 'Excl_rows_beforeModel'
  writexl::write_xlsx(ex,paste0(diroutput,"/ExclRowsBeforeModel_base",mybase,".xlsx")) #save in file
  # exclude
  dat <- dplyr::filter(dat , (Outliers_bcAmps_withinCond != 1) )
  
}

#Ensure format of your variables (as.factor or as.numeric)
dat$group1625_SLRTsep_OR <- as.factor(dat$group1625_SLRTsep_OR)
...
outputnamebase <- paste0('LMMCovaNix_',mytype,'_',mybase)
...
outcol[as.numeric(excludedRows_id),1] <- 1
...
if(exists('additionalContrasts')){ 
  ...
  newcon <- emmeans(fit,as.formula('pairwise ~ hemisphere | group1625_SLRTsep_OR'),adjust="Tukey") 
  additionalContrasts[[length(additionalContrasts)+1]]  <- newcon
  newcon <- emmeans(fit,as.formula('pairwise ~ group1625_SLRTsep_OR | hemisphere'),adjust="Tukey") 
  additionalContrasts[[length(additionalContrasts)+1]]  <- newcon
  newcon <- emmeans(fit,as.formula('pairwise ~  group1625_SLRTsep_OR | hemisphere | cond'),adjust="Tukey") 
  additionalContrasts[[length(additionalContrasts)+1]]  <- newcon  
