...
for (t in 1:length(tasks))  {
        ...
        resultsfile <- paste0('O:/studies/grapholemo/analysis/LEMO_GFG/mri/2ndLevel/???FeedbackLearning/Result_regions_2Lv_regreNeg_GLM0_thirds_exMiss_lgvt_speed_pr.xlsx')
        epifolder <- paste0('O:/studies/grapholemo/analysis/LEMO_GFG/mri/2ndLevel/FeedbackLearning/',task,'/2Lv_regreNeg_GLM0_thirds_exMiss_lgvt_speed_pr')
        
        
        
        epifiles <- dir(epifolder,pattern="spmT_0001.nii",recursive=TRUE)
        
        diroutput <- 'O:/studies/grapholemo/analysis/LEMO_GFG/mri/visualizations_mricrogl/fbl_regressors/'
        ...
        # Do the stuff only if results for this task were found 
        if (dim(dat)[1]!=0 ){
            ...
            contrast <- unique(substr(dat$file,
                                      unlist(gregexpr('con0',dat$file[1])),
                                      unlist(gregexpr('con0',dat$file[1]))+6 )) # find pattern in file name ssuming the all en with *con_xxxxx_1.xlsx
            
            for (c in 1:length(contrast)){
              
              # read results table
              curdat <- dat[grep(contrast[c],dat$file),]
              if (any(as.numeric(curdat$cluster_k) > 15)){
                curdat <- curdat[which(as.numeric(curdat$cluster_k) > 15),]  
              }
