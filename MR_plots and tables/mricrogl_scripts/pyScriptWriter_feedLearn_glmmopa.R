...
for (t in 1:length(tasks))  {
        ...
        resultsfile <- paste0('O:/studies/grapholemo/analysis/LEMO_GFG/mri/2ndLevel/FeedbackLearning/Result_regions_2Lv_GLM0_mopa_aspe_withFWE.xlsx')
        
        epifolder <- paste0('O:/studies/grapholemo/analysis/LEMO_GFG/mri/2ndLevel/FeedbackLearning/',task,'/LEMO_rlddm_v32/2Lv_GLM0_mopa_aspe/')
        epifiles <- dir(epifolder,pattern="spmT_0001.nii",recursive=TRUE) #
        
        diroutput <- 'O:/studies/grapholemo/analysis/LEMO_GFG/mri/visualizations_mricrogl/fbl_mopa/'
        ...
        allscripts <- list()
        #contrast <- unique(substr(dat$file, unlist(gregexpr('con_',dat$file[1])),unlist(gregexpr('con_',dat$file[1]))+7 )) # find pattern in file name assuming the all en with *con_xxxxx_1.xlsx
        contrast <- c('con_0006','con_0007','con_0008','con_0009')
        for (c in 1:length(contrast)){
          
          # read results table
          curdat <- dat[grep(contrast[c],dat$file),]
          if (any(as.numeric(curdat$cluster_k) > 30)){
            curdat <- curdat[which(as.numeric(curdat$cluster_k) > 30),]  
          }
