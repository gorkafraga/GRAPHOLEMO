...
audioseq <- list()
for (i in 1:4) {
  seq <- sample(rep(1:8,nreps))
  ii <-0 
    while (length(which(diff(sort(c(which(seq==1),c(which(seq==2)))))==1)>1) ||
           length(which(diff(sort(c(which(seq==3),c(which(seq==4)))))==1)>1) ||
           length(which(diff(sort(c(which(seq==5),c(which(seq==6)))))==1)>1) ||
           length(which(diff(sort(c(which(seq==7),c(which(seq==8)))))==1)>1) ||
...
for (b in 1:length(audioseq)){
  ...
  tmp<- sample(which(audioseq[[b]]==7))
  visualseq[[b]][tmp[1:(nreps/2)]] <- 7
  visualseq[[b]][tmp[1+(nreps/2):length(tmp)]] <- 8
  
  tmp<- sample(which(audioseq[[b]]==8))
  visualseq[[b]][tmp[1:(nreps/2)]] <- 8
  visualseq[[b]][tmp[1+(nreps/2):length(tmp)]] <- 7
...
setwd(diroutput)
xlsx::write.xlsx(table2save,'Seqs_UVALEMO_A_6x8stim.xlsx',row.names = TRUE)
