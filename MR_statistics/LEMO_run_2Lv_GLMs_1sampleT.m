...
for c = 1: length(contrast) 
    ...
    for sub = 1:length(subjects)
            matlabbatch{1}.spm.stats.factorial_design.des.pt.pair(sub).scans = {
                                                                           ['O:\studies\grapholemo\analysis\LEMO_GFG\mri\1stLevel\FBL_A\',listFolders(indx).name,'\',subjects{sub},'\',contrastname,'.nii,1']
                                                                            ['O:\studies\grapholemo\analysis\LEMO_GFG\mri\1stLevel\FBL_B\',listFolders(indx).name,'\',subjects{sub},'\',contrastname,'.nii,1']
                                                                           };
    end
