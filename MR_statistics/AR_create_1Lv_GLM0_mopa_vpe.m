function [matlabbatch] =  AR_create_1Lv_GLM0_mopa_vpe(pathSubject,modeloutput,scans,onsets,nsessions)
if nargin < 1
    ...
    for s=1:nsessions
       ...
       %PEnum = cell2mat(cellfun(@(S) sscanf(S, '%f%f%f%f%f%f%f%f').',modeloutput{s}.pe_tot_hat,'uniform', 0)); % transform to double PE values
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(s).cond(1).pmod.name = 'drift';
    ...
    matlabbatch{3}.spm.stats.con.consess{6}.tcon.name = 'Stim posDrift';
    matlabbatch{3}.spm.stats.con.consess{6}.tcon.weights = [0 1 0 0 0];
    matlabbatch{3}.spm.stats.con.consess{6}.tcon.sessrep = 'repl';
    
    matlabbatch{3}.spm.stats.con.consess{7}.tcon.name = 'Stim negDrift';
