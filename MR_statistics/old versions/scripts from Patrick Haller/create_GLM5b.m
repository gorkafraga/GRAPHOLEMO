...
function matlabbatch = create_glm_01c_sc_fppos(paths, task, subject,scans,rp,bad_scans)
...
pathSubject = fullfile(paths.study,paths.analysis,task,'1stlev_01c',subject);
...
for s=1:3
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(s).cond(2).pmod.name = 'PE_pos';
    matlabbatch{1}.spm.stats.fmri_spec.sess(s).cond(2).pmod.param = cell2mat(parameters(5,s));
...
matlabbatch{3}.spm.stats.con.consess{5}.tcon.name = 'Parametric modulation of feedback onset by positive PE';
