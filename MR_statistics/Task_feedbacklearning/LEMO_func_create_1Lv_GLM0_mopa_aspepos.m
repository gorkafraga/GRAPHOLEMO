function [matlabbatch] =  LEMO_func_create_1Lv_GLM0_mopa_aspepos(pathSubject,modeloutput,scans,onsets,nsessions)
if nargin < 1
    ...
    for s=1:nsessions
        ...
        as_active = modeloutput{s}.as_active; % transform to double PE values
        PEnum = modeloutput{s}.pe_pos_hat; % transform to double PE values
        %Scans
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(s).cond(1).pmod.name = 'AS';
        matlabbatch{1}.spm.stats.fmri_spec.sess(s).cond(1).pmod.param = as_active-mean(as_active); %  
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(s).cond(2).pmod.name = 'PEpos';% prediction error 
