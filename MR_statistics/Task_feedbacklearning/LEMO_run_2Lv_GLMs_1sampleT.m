...
selectedGLM = 'GLM0_mopa_aspe'; %selectedGroup = 'all';
...
if  contains(selectedGLM,'_mopa')
    ...
    basedir1 = ['O:\studies\grapholemo\analysis\LEMO_GFG\mri\1stLevel\FBL_A\',modelversion,'\'];
    basedir2 = ['O:\studies\grapholemo\analysis\LEMO_GFG\mri\1stLevel\FBL_B\',modelversion,'\'];
    
else
   % listFolders = dir(strcat(parentDir,'\',selectedGroup,'\1Lv_',selectedGLM,'*'));   
    listFolders = dir(strcat(parentDir,'\1Lv_',selectedGLM,'*'));   
    basedir1 = 'O:\studies\grapholemo\analysis\LEMO_GFG\mri\1stLevel\FBL_A\';
    basedir2 = 'O:\studies\grapholemo\analysis\LEMO_GFG\mri\1stLevel\FBL_B\';
end
...
diroutput = strrep(diroutput, '1stLevel\FBL_A\','2ndLevel_pairedTs\');
mkdir(diroutput)
...
for c = 1: length(contrast) 
    ...
    for sub = 1:length(subjects)
            matlabbatch{1}.spm.stats.factorial_design.des.pt.pair(sub).scans = {
                                                                           [basedir1,listFolders(indx).name,'\',subjects{sub},'\',contrastname,'.nii,1']
                                                                            [basedir2,listFolders(indx).name,'\',subjects{sub},'\',contrastname,'.nii,1']
                                                                           };
    end
    ...
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = ['(A > B) ', cell2mat(contrast(c).descript)];
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = [1 -1];
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
    matlabbatch{3}.spm.stats.con.consess{2}.tcon.name = ['(B > A) ', cell2mat(contrast(c).descript)];
