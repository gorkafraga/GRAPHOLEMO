rm(list=ls())
# RUN t-test over each column of the wide formatted dataset(e.g., roi betas)
#----------------------------------------------------------------------------
# -  Two tailed t-tests against 0 

# define inputs
dirinput <- 'O:/studies/grapholemo/analysis/LEMO_GFG/mri/stats_ROI/GLM0_mopa_aspe/'
diroutput <- dirinput
fileinput <- 'ROI_vals_2LvGLM0mopaaspe.sav'
setwd(dirinput)
...
onsets <- c('asPos','asNeg','pePos','peNeg')
tidyT <- list()
tasks <- c('FBLA','FBLB')
counter <- 1
