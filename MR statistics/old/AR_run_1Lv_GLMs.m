...
selectedGLM = 'GLM1_pm1a';
%Specifies the folders assigned to each GLM 
choices = containers.Map({'GLM0','GLM1','GLM1_pm1a','GLM2'},...
...
paths.analysis =  choices(selectedGLM);  
...
subjects = subjects(~contains(subjects,'AR1026'));
subjects = subjects(~contains(subjects,'AR1016'));
subjects = subjects(~contains(subjects,'AR1063'));
%subjects = {'AR1016'}
%subjects = {'AR1005'};
%spm fmri
spm('defaults','fMRI')
for i =2:length(subjects)
  ...
  elseif (find(cellfun(@isempty,{onsets.fbOnset_neg})))
      ...
      if strcmp('GLM0',selectedGLM)
        matlabbatch = AR_create_1Lev_GLM0(pathSubject,scans,onsets,nsessions);
      elseif   strcmp('GLM1',selectedGLM)
         matlabbatch = AR_create_1Lev_GLM1(pathSubject,scans,onsets,nsessions);
      elseif  strcmp('GLM1_pm1a',selectedGLM)
         matlabbatch = AR_create_1Lev_GLM1_pm1a(pathSubject,scans,onsets,params,nsessions);
      end
