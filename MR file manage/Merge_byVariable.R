...
perfofile = 'O:/studies/allread/mri/analysis_GFG/stats/task/performance/learn_12_allFromNada/Perform_summary.xlsx'

# read master file and performance data
masterData <- xlsx::read.xlsx(masterfile,sheetName = 'IDs_Demographics',detectDates = TRUE)
perfoData <-   xlsx::read.xlsx(perfofile,sheetIndex = 1,detectDates = TRUE)

#
Dmerged <- merge.data.frame(masterData[,1:2],perfoData, by = "subjID", all.x = TRUE, all.y = TRUE, sort = TRUE)
