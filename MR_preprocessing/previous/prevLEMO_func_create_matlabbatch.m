function [matlabbatch] = LEMO_func_create_matlabbatch_GFG(paths,currTask,currsubject,anatTemplate)
...
if strcmp(currTask,'eread')
    scansList = { cellstr(spm_select('ExtFPList',epiDir, '^mr.*.eread_366.nii$', Inf)) };
    nslices = 32;
    tr = 1.000;
    blocks = 1;
...
elseif strcmp(currTask,'localizer')
    scansList = { cellstr(spm_select('ExtFPList',epiDir, '^mr.*.epi_localizer.*.nii$', Inf)) };
    nslices = 42;
    tr = 1.250; 
    blocks = 1;
    smoothSize = 6; % smoothing factor in mm
