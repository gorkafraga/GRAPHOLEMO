...
%Specifies the folders assigned to each GLM 
 ...
 diroutputChoices = containers.Map({'GLM0','GLM1','GLM1_pm1a','GLM0_halfs','GLM0_quartiles','GLM0_positiv_first10_last10','GLM0_thirds','GLM0_mopa'},...
        ...
        'O:\studies\allread\mri\analyses_NF\mri_analyses_NF\first_level_NF\_scripts\model_based\dw-rlddm4'});   
    
% Set up directories and some variables 
paths.analysis =  diroutputChoices(selectedGLM);  
addpath ('O:\studies\allread\mri\analyses_NF\mri_analyses_NF\first_level_NF\_scripts');% set path to this script and associated functions
addpath ('O:\studies\allread\mri\analyses_NF\mri_analyses_NF\first_level_NF\_scripts\model_based');% set path to this script and associated functions
...
paths.logs = 'O:\studies\allread\mri\analyses_NF\Logs\2blocks_logs_verygoodperf'; % here the selected files for this analysis, suffix was fixed when needed so all end in  _bX.txt
...
for i = 1:numel(subjects)
    ...
    % cd(pathSubject)
    logfileID = fopen([pathSubject '\LOG_firstLevel.txt'],'w');
...
elseif strcmp('GLM0_mopa',selectedGLM)
         matlabbatch = AR_create_1Lv_GLM0_mopa_dw(pathSubject,modeloutput,scans,onsets,nsessions);
else 
...
return % comment me to run automatically



if (isempty(gcp('nocreate')))
    fprintf('No parallel pool is running. Creating a parpool with 8 cores.\n');
    parpool(8);
end    
