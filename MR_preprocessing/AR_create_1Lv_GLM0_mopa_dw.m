% this version does not format the parameter table, needed for patricks
% ouput tables
function [matlabbatch] =  AR_create_1Lv_GLM0_mopa_dw(pathSubject,modeloutput,scans,onsets,nsessions)
if nargin < 1
    ...
    for s=1:nsessions
        ...
        %ASnum = cell2mat(cellfun(@(S) sscanf(S, '%f%f%f%f%f%f%f%f').',modeloutput{s}.as_chosen,'uniform', 0)); % transform to double PE values
        %PEnum = cell2mat(cellfun(@(S) sscanf(S, '%f%f%f%f%f%f%f%f').',modeloutput{s}.pe_tot_hat,'uniform', 0)); % transform to double PE values
        ASnum = modeloutput{s}.as_chosen; % transform to double PE values
        PEnum = modeloutput{s}.pe_tot; % transform to double PE values
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(s).cond(1).pmod.param = ASnum-mean(ASnum); % HERE TAKE AS VALUES FOR CHOICE
