function [matlabbatch] = LEMO_func_preproc_segment(b0Dir,epiDir,t1Dir,currTask,anatTemplate)
...
%-----------------------------------
   ...
   %% Segmentation  (time consuming. note if running several task batches in parallel may crash)
    ...
    matlabbatch{5}.spm.util.imcalc.options.dtype = 16;
       ...
       %% CORREGISTRATION 
    %--------------------------  
    matlabbatch{6}.spm.spatial.coreg.estimate.ref(1) = cellstr(spm_select('FPList', t1Dir, '^im.*.nii$')); 
    ...
    %------------------------------------------------------------------------------------
    matlabbatch{6+1}.spm.spatial.normalise.write.subj.def(1) = cellstr(spm_select('FPList', t1Dir, '^y_.*.nii$')); %cfg_dep('Segment: Forward Deformations', substruct('.','val', '{}',{3}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','fordef', '()',{':'}));
    matlabbatch{6+1}.spm.spatial.normalise.write.subj.resample(1) =  cfg_dep('Coregister: Estimate: Coregistered Images', substruct('.','val', '{}',{6}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','cfiles'));% cellstr(spm_select('FPList', epiDir,'^uaGPL.*.nii')) ; % cfg_dep('Coregister: Estimate: Coregistered Images', substruct('.','val', '{}',{6}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','cfiles'));
    matlabbatch{6+1}.spm.spatial.normalise.write.woptions.bb = [-90 -126 -72
                                                                91 91 109];  %keep this 'box'  in 2 lines!
    matlabbatch{6+1}.spm.spatial.normalise.write.woptions.vox = [voxelSize voxelSize voxelSize]; % change this 
    matlabbatch{6+1}.spm.spatial.normalise.write.woptions.interp = 7;
    matlabbatch{6+1}.spm.spatial.normalise.write.woptions.prefix = 'w';  
    matlabbatch{6+2}.spm.spatial.smooth.data(1) = cfg_dep('Normalise: Write: Normalised Images (Subj 1)', substruct('.','val', '{}',{6+1}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('()',{1}, '.','files'));
    matlabbatch{6+2}.spm.spatial.smooth.fwhm = [smoothSize smoothSize smoothSize]; 
    matlabbatch{6+2}.spm.spatial.smooth.dtype = 0;
    matlabbatch{6+2}.spm.spatial.smooth.im = 0;
    matlabbatch{6+2}.spm.spatial.smooth.prefix = ['s',num2str(smoothSize)];
    matlabbatch{6+3}.spm.spatial.normalise.write.subj.def(1) =cellstr(spm_select('FPList', t1Dir, '^y_.*.nii$'));
    matlabbatch{6+3}.spm.spatial.normalise.write.subj.resample(1) = cellstr(spm_select('FPList', t1Dir, '^im.*.nii$')); 
    matlabbatch{6+3}.spm.spatial.normalise.write.woptions.bb = [-90 -126 -72
                                                                91 91 109]; %keep this in 2 lines!
    matlabbatch{6+3}.spm.spatial.normalise.write.woptions.vox = [1 1 1];
    matlabbatch{6+3}.spm.spatial.normalise.write.woptions.interp = 7;
    matlabbatch{6+3}.spm.spatial.normalise.write.woptions.prefix = 'w';
