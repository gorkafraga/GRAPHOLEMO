//https://github.com/CCS-Lab/hBayesDM/blob/master/commons/stan_files/bandit2arm_delta.stan
...
data {
  ...
  int<lower=1> first [N];
  int<lower=1> last[N];
  int<lower=1, upper=T> Tsubj[T];
  int<lower=-1, upper=2> choice[T];
  real outcome[T];  // no lower and upper bounds
}
transformed data {
...
model {
  ...
  for (i in 1:N) { 
    ...
    for (t in (first[i]):(last[i])) {
      ...
      choice[t] ~ categorical_logit(tau[i] * ev);
    ...
    for (i in 1:N) {
     ...
     // real PE;      // prediction error
      ...
      for (t in (first[i]):(last[i])) {
        ...
        log_lik[i] += categorical_logit_lpmf(choice[t] | tau[i] * ev);
        
        // generate posterior prediction for current trial
        y_pred[i,t] = categorical_rng(softmax(tau[i] * ev));
        
        // prediction error
        PE[t] = outcome[t] - ev[choice[t]];
        
        // value updating (learning)
        ev[choice[t]] += A[i] * PE[t];
      }
