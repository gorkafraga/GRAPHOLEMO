data {
  ...
  real trials[N];      // trials per subject
...
model {
  ...
  for (s in 1:N) {
    ...
    for(trial in (first[s]):(last[s]-1)) {
      ...
      // if lower bound
      if (response[trial]==1){
        v[trial] = -((ev[trial,stim_assoc[trial]] + ev[trial,stim_nassoc[trial]])/2 * v_mod[s]); // Response = 1 means incorrect. So drift is (-)
        RT[trial] ~  wiener(a[s] * pow(iter[trial]/10,a_mod[s]),tau[s] ,0.5,v[trial]);  // s
      ...
      // if upper bound (resp = 2)
        ...
        v[trial] = (ev[trial,stim_assoc[trial]] + ev[trial,stim_nassoc[trial]])/2 * v_mod[s]; // Response = 2 means correct. So drift is (+)
    ...
    if (response[last[s]]==1){
      ...
      RT[last[s]] ~  wiener(a[s] * pow(iter[last[s]]/10,a_mod[s]),tau[s] ,0.5,v[last[s]]);
    }
...
generated quantities {
  ...
  for (s in 1:N){
    ...
    if (response[last[s]]==1){
      ...
      pe_pos_hat[last[s]] = 0;
      log_lik[last[s]] = wiener_lpdf(RT[last[s]] | a[s] * pow(iter[last[s]]/10,a_mod[s]),tau[s],0.5,v_hat[last[s]]);
    }
    else{
      v_hat[last[s]] = (ev_hat[last[s]-1,stim_assoc[last[s]]] + ev_hat[last[s]-1,stim_nassoc[last[s]]])/2 * v_mod[s];
