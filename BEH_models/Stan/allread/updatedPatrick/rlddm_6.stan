...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N] eta; // learning parameter
  ...
  for (s in 1:N) {
    ...
    for(trial in (first[s]):(last[s]-1)) {
      ...
      if (response[trial]==1){
        ...
        RT[trial] ~  wiener(a[s] ,tau[s] ,z, v[trial]);
        //ev[trial+1,stim_nassoc[trial]] = ev[trial,stim_nassoc[trial]] + (eta[s] * fabs(value[trial]-(1-ev[trial,stim_assoc[trial]])));
      ...
      // if upper bound (resp = 2)
        ...
        v[trial] = (ev[trial,stim_assoc[trial]] + ev[trial,stim_nassoc[trial]])/2 * v_mod[s];
        RT[trial] ~  wiener(a[s] ,tau[s],z, v[trial]);
        //ev[trial+1,stim_nassoc[trial]] = ev[trial,stim_nassoc[trial]] + (eta[s] * (value[trial]-(1-ev[trial,stim_assoc[trial]])));
        //if (ev[trial+1,stim_nassoc[trial]]>1){
        //  ev[trial+1,stim_nassoc[trial]]=1;
        //}
    ...
    if (response[last[s]]==1){
      ...
      RT[last[s]] ~  wiener(a[s] ,tau[s] ,z, v[last[s]]);
    }
...
generated quantities {
  ...
  real<lower=0> mu_eta;                 // learning rate 
  ...
  for (s in 1:N){
    ...
    for(trial in (first[s]):(last[s]-1)) {
      ...
      if (response[trial]==1){
        ...
        pe_pos_hat[trial] = 0;
        //ev_hat[trial+1,stim_nassoc[trial]] = ev_hat[trial,stim_nassoc[trial]] + (eta[s] * fabs(value[trial]-(1-ev_hat[trial,stim_assoc[trial]])));
      ...
      // if upper bound (resp = 2)
        ...
        pe_neg_hat[trial] = 0;
        //ev_hat[trial+1,stim_nassoc[trial]] = ev_hat[trial,stim_nassoc[trial]] + (eta[s] * (value[trial]-(1-ev_hat[trial,stim_assoc[trial]])));
        //if (ev_hat[trial+1,stim_nassoc[trial]]>1){
        //  ev_hat[trial+1,stim_nassoc[trial]]=1;
        //}
