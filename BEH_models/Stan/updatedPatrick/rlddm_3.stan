  ...
  for (s in 1:N) {
    ...
    for(trial in (first[s]):(last[s]-1)) {
      ...
      if (response[trial]==1){
        ...
        //ev[trial+1,stim_nassoc[trial]] = ev[trial,stim_nassoc[trial]] + (eta_neg[s] * fabs(value[trial]-(1-ev[trial,stim_assoc[trial]])));
      ...
      // if upper bound (resp = 2)
        ...
        v[trial] = (ev[trial,stim_assoc[trial]] + ev[trial,stim_nassoc[trial]])/2 * v_mod[s];
        RT[trial] ~  wiener(a[s] ,tau[s],z, v[trial]);
        //ev[trial+1,stim_nassoc[trial]] = ev[trial,stim_nassoc[trial]] + (eta_pos[s] * (value[trial]-(1-ev[trial,stim_assoc[trial]])));
        //if (ev[trial+1,stim_nassoc[trial]]>1){
        //  ev[trial+1,stim_nassoc[trial]]=1;
        //}
