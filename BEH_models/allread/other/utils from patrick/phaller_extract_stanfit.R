...
extract_stanfit <- function(stanfit,meancenter){
  ...
  file <- dir(path = source,pattern="_data.Rda")
  ...
  write.table(subject_parameters, paste0("outputs/","rlddm_subject_parameters.csv"), sep=",", row.names=FALSE, col.names=TRUE,quote = FALSE) 
