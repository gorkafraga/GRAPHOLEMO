...
dirinput <- "O:/studies/grapholemo/analysis/LEMO_GFG/beh/modeling/Logs_preprocessed_n39"
...
model  <- c('lemo_bandit2arm_delta')   
diroutput <- paste0(dirinput,'/',model,'/output')
...
datList$outcome[which(datList$outcome==0)] <- -1
    ...
    dir.create(paste0(dirinput,'/',model,'/input'))
    # save 
    save(datList,file=paste0(dirinput,'/',model,'/input/Preproc_list.rda'))
    save(datTable,file=paste0(dirinput,'/',model,'/input/Preproc_data.rda')) 
    write.csv(datTable,file=paste0(dirinput,'/',model,'/input/Preproc_data.csv'),row.names = FALSE)
    write(subjects,file=paste0(dirinput,'/',model,'/input/Preproc_subjects.txt'))
 

# Sampling settings ----------------------------------------------------
 mychains <- 4 # numer of mcmc chains
 myiter <-  300#10000
 mywarmup <-  100#4000 # in cmd stan these are not part of the iterations This is also called 'burn-in' 
 init_list <- NULL
