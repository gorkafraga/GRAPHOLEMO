...
parameters {
  ...
  vector[N] eta_pos_pr; // learning rate correct trials
  vector[N] eta_neg_pr;// learning rate incorrrect trials
...
transformed parameters {
 ...
 // Declare
  ...
  //Constrain
  eta_pos = 0.05*Phi_approx(mu_eta_pr[1] + sigma_eta_pr[1] * eta_pos_pr);  
  eta_neg = 0.05*Phi_approx(mu_eta_pr[2] + sigma_eta_pr[2] * eta_neg_pr);
  
  a = exp(mu_pr[1] + sigma_pr[1] * a_pr); 
  a_mod = exp(mu_pr[2] + sigma_pr[2] * a_mod_pr);
  v_mod = exp(mu_pr[3] + sigma_pr[3] * v_mod_pr);
  for (s in 1:N) {
    tau[s]  = Phi_approx(mu_pr[4] + sigma_pr[4] * tau_pr[s]) * (minRT[s] - RTbound) + RTbound; // tau depends upon subjects' minimum RT 
    tau[s] = fabs(tau[s]);
  } 
}
// ------------------------------------------------------------
model {
 ...
 // GROUP LEVEL priors 
  ...
  sigma_eta_pr ~ normal(0, 0.5); ////// GORKA MENTAL NOTE: why didn't it take sigma_eta_pr  as name??? 
    ...
    for(trial in (first[s]):(last[s]-1)) {
     ...
     // if lower bound (incorrect resp,that is response= 1)
      if (response[trial]==1){
        ...
        ev[trial+1,stim_nassoc[trial]] = ev[trial,stim_nassoc[trial]] + (eta_neg[s] * fabs(value[trial]-(1-ev[trial,stim_nassoc[trial]]))); // adjust expected values for each stimuli pair in this trial
  ...
  for (s in 1:N){
    ...
    for(trial in (first[s]):(last[s]-1)) {
      ...
      if (response[trial]==1){
        ...
        log_lik[trial] = wiener_lpdf(RT[trial] | a[s] * pow(iter[trial]/10,a_mod[s]),tau[s],beta,v_hat[trial]);
      }
      // if upper bound (resp = 2)
      else{
        v_hat[trial] = (ev_hat[trial,stim_assoc[trial]] + ev_hat[trial,stim_nassoc[trial]])/2 * v_mod[s];
    ...
    if (response[last[s]]==1){
      ...
      pe_pos_hat[last[s]] = 0;
      log_lik[last[s]] = wiener_lpdf(RT[last[s]] | a[s] * pow(iter[last[s]]/10,a_mod[s]),tau[s],beta,v_hat[last[s]]);
    }
    else{
      v_hat[last[s]] = (ev_hat[last[s]-1,stim_assoc[last[s]]] + ev_hat[last[s]-1,stim_nassoc[last[s]]])/2 * v_mod[s];
