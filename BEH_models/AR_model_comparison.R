...
rm(list=ls(all=TRUE)) 
Packages <- c('rstanarm','rstudioapi',"grid","gridExtra","ggplot2")
lapply(Packages, require, character.only = TRUE)
# Edit inputs
dirinput <- "G:/Nada_may2021"
diroutput <- "G:/Nada_may2021"
readfit <- 0 
...
if (readfit==1) {
  for (i in 1:length(listModels)){
    fit <- readRDS(paste0(dirinput,'/',listModels[i],'.rds'))
    loglik1 <- loo::extract_log_lik(fit, merge_chains = FALSE)
    saveRDS(loglik1,paste0(dirinput,'/',listModels[i],'_logLik.rds'))
  }
